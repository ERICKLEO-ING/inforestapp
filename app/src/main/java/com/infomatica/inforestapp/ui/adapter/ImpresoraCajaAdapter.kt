...
class ImpresoraCajaAdapter(
    private val mItemListener: ItemClickListener,
) : ListAdapter<ImpresoraCajaModel, ImpresoraCajaAdapter.ViewHolder>(DiffCallback()) {
...
//        holder.binding.tvdescripcionalerta.text = item.mensaje
    ...
    interface ItemClickListener {
        fun onItemClick(details: ImpresoraCajaModel?)
    }
    inner class ViewHolder(val binding: ItemTarjetaBinding) : RecyclerView.ViewHolder(binding.root)

    class DiffCallback : DiffUtil.ItemCallback<ImpresoraCajaModel>() {
        override fun areItemsTheSame(oldItem: ImpresoraCajaModel, newItem: ImpresoraCajaModel): Boolean {
